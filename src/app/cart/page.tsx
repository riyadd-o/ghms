          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="rounded-lg border border-luxury-gold/10 bg-luxury-green-secondary/30 p-6 flex flex-col">
                ...
                <input type="number" inputMode="numeric" pattern="[0-9]*" placeholder={locationType === "Table" ? "Please enter your table number" : "Please enter your room number"} value={locationNumber} onKeyDown={(e) => { if (['e', 'E', '+', '-', '.'].includes(e.key)) e.preventDefault(); }} onChange={(e) => { const numericValue = e.target.value.replace(/[^0-9]/g, ""); setLocationNumber(numericValue); if (tableError) setTableError(""); }} className={`w-full rounded border ${tableError ? 'border-red-500' : 'border-luxury-gold/25'} bg-luxury-green/40 p-3 text-sm text-white placeholder-gray-600 outline-none focus:border-luxury-gold [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none`} />
